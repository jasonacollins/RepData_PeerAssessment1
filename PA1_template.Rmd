...
The mean of the total number of steps taken per day is `r format(meansteps, scientific=FALSE)`. The median of the total number of steps taken per day is `r format(mediansteps, scientific=FALSE)`.
...
intervalMax
```
The interval in which the maximum number of steps occurs is `r intervalMax$interval`, when `r round(intervalMax$steps, 0)` steps are taken.
...
For the complete dataset with missing values imputed, the mean of the total number of steps taken per day is `r format(meanstepsImputed, scientific=FALSE)`. The median of the total number of steps taken per day is `r format(medianstepsImputed, scientific=FALSE)`. (Note: if we had taken the mean across all days, this mean would not have changed from the earlier answer - and the median would have been equal to the mean - coming from one of the imputed days.)
